...
static Plan DecidePlan(const Probe& p) {
    ...
    if (plan.copyVideo && plan.copyAudio)      plan.label = L"换壳";
    else if (plan.copyVideo)                   plan.label = L"换壳 + 转音频";
    else                                       plan.label = L"重新编码";
    return plan;
}
